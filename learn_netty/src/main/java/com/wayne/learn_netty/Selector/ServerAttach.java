 ...
 * @Description: selector服务端=附件=扩容
 */
@Slf4j
public class ServerAttach {
        ...
        while (true){
            ...
            while (iterator.hasNext()){
                ...
                if(next.isAcceptable()){
                    ...
                    //将一个ByteBuffer 作为附件 关联到SelectionKey 上
                    SelectionKey registerKey = accept.register(selector, 0, buffer);
                    registerKey.interestOps(SelectionKey.OP_READ);
                }else if(next.isReadable()){
                    try{
                        SocketChannel channel = (SocketChannel)next.channel();
                        //在同一个SelectionKey 获取到关联的ByteBuffer
